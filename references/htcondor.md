...
- **Black-hole nodes**: a single broken execute node (bad mount, missing group/filesystem access, misconfig) often *fails jobs instantly*, which frees its slots immediately so the negotiator keeps feeding it — one bad node can eat a large share of a sweep, and `periodic_release` makes it worse by re-matching there. When many jobs hold or fail, **group the reason by machine first**: `condor_q -hold -af HoldReason | grep -oE '<hostpattern>' | sort | uniq -c`. If it's concentrated on one node, exclude it: `requirements = ... && (Machine =!= "badnode.domain") && (Machine =!= LastMatchName0)` — the second clause is general black-hole avoidance (a released job won't return to the node it just failed on).
